<template>
  <div class="svg"></div>
  <div class="topbar">
    <n-button @click="goback" strong quaternary round>
      <n-icon>
        <return-up-back />
      </n-icon>
    </n-button>
    <text style="line-height: 50px; color: #383838; margin-right: 20px"
      >标题</text
    >
    <n-input
      ...
      round
      placeholder="请输入标题"
      style="width: 1000px; background-color: #f3f0f9"
    />
  ...
  <div class="tabs">
    <n-card>
      <rich-text-editor
        v-model:modelValue="addArticle.content"
      ></rich-text-editor>
    </n-card>
  </div>
      ...
      <n-card title="封面" :bordered="false">
        ...
        <div v-else style="width: 230px; margin: 0 auto">
          <n-image
            height="150"
            width="300"
            :src="serverUrl + addArticle.headImage"
          />
          <n-button
            @click="deleteImage"
            circle
