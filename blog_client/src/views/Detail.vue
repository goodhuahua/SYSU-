  ...
  <div class="tabs">
    <n-card>
      ...
      <div style="height: 75px; background-color: #fcfaf7">
        ...
        />
        <text style="position: relative; left: 36px; color: #808080"
          >发布时间：{{ articleInfo.created_at }}
        </text>
        ...
        <n-button
          ...
          @click="toDelete"
          ghost
          style="bottom: 45px; left: 815px"
          >删除</n-button
        >
        <n-button
          style="bottom: 45px; left: 550px"
          round
          ghost
          @click="toComments"
        >
      ...
      <n-button
        ...
        text
        @click="newCollect"
        style="position: absolute; right: 40px; top: 25px; cursor: pointer"
        round
        ghost
        color="#FFA876"
      >
...
const toComments = async () => {
  await router.push({ path: "/comments", query: { id: articleInfo.value.id } });
};
...
const loadArticle = async () => {
  ...
  if (res1.data.code == 200) {
    ...
    let res2 = await axios.get(
      "category/" + res1.data.data.article.category_id
    );
    console.log(res2);
    ...
    let res3 = await axios.get(
      "user/briefInfo/" + res1.data.data.article.user_id
    );
    console.log(res3);
