<template>
  <div class="topbar">
    <n-button
      ...
      quaternary
      round
      style="position: absolute; left: 500px; top: 7px; font-size: 24px"
      color="#046b1e"
    >
      <n-icon>
        <return-up-back />
    ...
    <text
      style="position: absolute; left: 200px; line-height: 50px; color: #000000"
      >标题</text
    >
    <n-input
    ...
    <n-avatar
      ...
      size="medium"
      :src="user.avatarUrl"
      style="position: absolute; right: 190px; top: 8px"
    />
    <div style="position: absolute; right: 50px; top: 8px">
      <n-button round color="#046b1e" @click="showModalModal">
        <template #icon>
          <n-icon>
            <send />
          </n-icon>
  ...
  <div class="tabs">
    <n-card>
      <rich-text-editor
        v-if="loadOk"
        v-model:modelValue="updateArticle.content"
      ></rich-text-editor>
    </n-card>
  </div>
      ...
      <n-card title="封面" :bordered="false">
        ...
        <div v-else style="width: 230px; margin: 0 auto">
          <n-image
            height="150"
            width="300"
            :src="serverUrl + updateArticle.headImage"
          />
          <n-button
            @click="deleteImage"
            circle
